...
class State:
    ...
    def save(self, filepath):
        ...
        arguments = dict((key, getattr(self.model, key)) for key in dir(self.model) if not callable(
            getattr(self.model, key)) and not key.startswith('_') and not key == 'kwargs')
    ...
    def load(filepath):
        ...
        state = State(model, checkpoint['optimizer'],
                      checkpoint['scheduler'], checkpoint['epoch'])

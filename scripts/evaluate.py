...
def run(args):
    ...
    model = ResNet(args.n_classes, [3, 32, 32], channels=12, blocks=[
                              3, 3, 3], clamp=True)
    ...
    # model.load_state_dict(torch.load(args.saved_model_file)())
    model.load_state_dict(torch.load(args.saved_model_file))
    model.to(device)
    model.eval()
    if args.cuda:
...
def main():
    ...
    parser.add_argument('--cuda', action='store_true', default=True, #default=False, from actual code 
                        help='Flag to enable running on GPU')
    parser.add_argument('--epsilon', type=float, default=0.1,
                        help='Learning rate for attack')
    parser.add_argument('--mask_pos', type=int, nargs='+', default= (1, 3), # Adding Default
                        help='Coordinates of top-left corner of mask in (y, x) format')
    parser.add_argument('--mask_dims', type=int, nargs='+', default= (4, 4), # Adding Default
                        help='Dimensions of mask in (h, w) format')
    parser.add_argument('--mode', type=str, 
                        choices={'clean', 'adversarial', 'all'}, default='all', help='Mode for evaluation')
    parser.add_argument('--dataset', type=str, default= "kaggle_gtsrb",
                        required=False, help='Dataset to use') # required=True
    ...
    parser.add_argument('--log_dir', type=str, required=False, default= "home/gopalknookula/Adversarial_Patch_Training/logs",  # Adding default
                        help='Directory to log tensorboard summary. Directory base path to be specified in common/paths.py.')
    parser.add_argument('--models_dir', type=str, required=False, default=  "home/gopalknookula/Adversarial_Patch_Training/experiments", 
                        ...
                        # Adding default to --models_dir and changing requried to false
    parser.add_argument('--signed_grad', action='store_true', default=False,
                        help='Flag to use sign of gradient when updating patch')
    parser.add_argument('--saved_model_file', type=str, default="experiments/adversarial_model_complete_130.pt",
    ...
    parser.add_argument('--use_tensorboard', action='store_true', #Changing default to True
                        default=True, help='Flag to enable Tensorboard logging')
    ...
    parser.add_argument('--n_classes', type=int, required=False, default=43,

...
def create_hdf5(img_h5_path, label_h5_path, img_label_path, sep=',', keys='tensor'):
    ...
    df = pd.read_csv(img_label_path, sep=sep, names=['label', 'name'], header=0) #name, label
    ...
    path_prefix = '/home/gopalknookula/Adversarial-Patch-Training/dataset/GTSRB'

    images = np.empty((len(img_paths),) + tuple(transform(Image.open(
        path_prefix + '/' + img_paths[0]).resize((32, 32)).convert('RGB')).permute(1, 2, 0).shape), dtype=np.float32)

    for idx, img_path in enumerate(img_paths):
        img = transform(Image.open(path_prefix + '/' + img_path).resize((32, 32)).convert('RGB'))
...
def main():
    ...
    parser.add_argument('--train_csv', type=str, default='/home/gopalknookula/Adversarial-Patch-Training/kaggle_gtsrb/Train.csv', #None ,
                        help='Path to CSV for training data')
    parser.add_argument('--test_csv', type=str, default='/home/gopalknookula/Adversarial-Patch-Training/kaggle_gtsrb/Test.csv', #None ,
                        help='Path to CSV for test data')
    parser.add_argument('--dataset', type=str, default= "kaggle_gtsrb",
                        required=False, help='Name of dataset')

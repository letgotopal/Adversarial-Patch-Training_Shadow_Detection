...
def run(args):
    ...
    # Train model
    print(initial_epoch + 1, args.epochs)
    for e in range(initial_epoch + 1, args.epochs):
        trainer.step(e)
        writer.flush()

        # Save model snapshot
        if (e + 1) % args.snapshot_frequency == 0 and args.mode == "normal":
            torch.save(model.state_dict(), f'experiments/model_{args.epochs}.pt')
        
        if (e + 1) % args.snapshot_frequency == 0 and args.mode == "adversarial":
            torch.save(model.state_dict(), f'experiments/adversarial_model_{args.epochs}.pt')
            #State.checkpoint(paths.experiment_file(
            #    args.models_dir, args.model_prefix + '_' + str(e + 1)), model, optimizer, scheduler, e)

    # Save final model
    if args.mode == "normal":
        torch.save(model.state_dict(), f'experiments/model_complete_{args.epochs}.pt')
    else:
        torch.save(model.state_dict(), f'experiments/adversarial_model_complete_{args.epochs}.pt')

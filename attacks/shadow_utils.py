...
# Support file for pso.py used to implement shadow_attack.py
...
def draw_shadow(position, image, pos_list, coefficient):
    ...
    image[x_list, y_list, 0] = np.round(image[x_list, y_list, 0] * coefficient).astype(
        np.uint8
    )
...
def random_param_generator(num, w, h):
    ...
    shadow_move = [
        [random.uniform(-15, 15), random.uniform(-15, 15)] for _ in range(num)
    ] + [[0, 0]]
    perspective_mat = [
        np.float32(
            [
                ...
                [w * random.uniform(0, 0.1), h * (1 - random.uniform(0, 0.1))],
            ]
        )
        for _ in range(num)
...
def image_transformation(
    ...
    for i in range(transform_num):
        ...
        adv_img, shadow_area = draw_shadow(
            shadow_position, image, pos_list, shadow_mul[i]
        )
        ...
        if (before - after).sum() != 0:
            ...
            adv_img = cv2.warpPerspective(
                adv_img, matrix, (w, h), borderMode=cv2.BORDER_REPLICATE
            )
            adv_img = adv_img[
                int(min(after[0:2, 1])) : int(max(after[2:4, 1])),
